...
>
> 🔐 **脱敏说明**：本文档不含真实账号 / ARN / 资源 ID。所有具体标识（Account ID、Pool ID、Gateway URL、Runtime ARN 等）由 Step 0 从本机的部署产物文件 `cognito_ids_ct.env`（已 gitignore，不入库）**自动加载为环境变量**；后续命令统一引用 `$POOL_ID`、`$GW_A_URL` 等变量，因此**照样可以直接复制执行**。
>
> **演示区域**：`us-east-1`
...
![部署架构](img/architecture-overview.svg)
...
> 下表中 `${...}` 均为 Step 0 从 `cognito_ids_ct.env` 加载的环境变量；真实值只在本机那个文件里，不入库。

| 资源 | 值（环境变量） |
|------|----|
| Cognito User Pool | `${POOL_ID}`（Essentials 功能层） |
| App Client ID | `${CLIENT_ID}`（无 secret，USER_PASSWORD_AUTH） |
| Pre-Token-Gen Lambda | `arn:aws:lambda:us-east-1:${ACCOUNT}:function:okx-ct-pretoken`（V2_0） |
| Target Lambda（3 工具） | `arn:aws:lambda:us-east-1:${ACCOUNT}:function:okx-ct-target` |
| 拦截器 Lambda | `arn:aws:lambda:us-east-1:${ACCOUNT}:function:okx-ct-interceptor` |
| **Gateway A（拦截器）** | `${GW_A_URL}` |
| **Gateway B（Cedar）** | `${GW_B_URL}` |
| Cedar Policy Engine | `${PE_ID}` |
| **Runtime**（指向 Gateway A） | `${RT_ARN}` |
| 3 个用户 | `viewer-user`(role_id=1001) / `analyst-user`(1002) / `trader-user`(1003)，密码见 `$DEMO_PASSWORD` |
...
# 从本机部署产物加载全部真实标识 (该文件已 gitignore, 不入库)
#   deploy_ct.sh 部署完成后会自动写出 cognito_ids_ct.env
set -a; source cognito_ids_ct.env; set +a
export DEMO_PASSWORD='OkxDemo#2026'

# 兜底: 账号 ID 若未在 env 里, 用当前凭证推导
export ACCOUNT="${ACCOUNT:-$(aws sts get-caller-identity --query Account --output text)}"

# 加载后可核对已就绪 (可选)
echo "POOL_ID=$POOL_ID  CLIENT_ID=$CLIENT_ID"
echo "GW_A_URL=$GW_A_URL"
echo "GW_B_URL=$GW_B_URL"
echo "RT_ARN=$RT_ARN  PE_ID=$PE_ID"
...
*本手册参数（Account ID / ARN / Gateway URL 等）均由 Step 0 从本机 `cognito_ids_ct.env` 加载，不写入文档；该文件在部署后生成、清理后失效，重新部署会写入新值。*
