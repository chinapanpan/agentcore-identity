...
- **RESPONSE 拦截器**（target 返回后）：对 `tools/list` 的结果**按组过滤**，无权限的工具对该用户**根本不可见**；
  并且**同时过滤 `result.structuredContent.tools`**（语义搜索路径），否则无权限工具会从该路径泄漏（详见 [BLOG_COMPARISON.md](BLOG_COMPARISON.md) §3）。
...
├── interceptor_unit_test.py # 拦截器本地单测 (合成 payload, 免部署, 22 用例)
...
├── cleanup.sh               # 一键清理全部资源
├── BLOG_COMPARISON.md       # 与 AWS 官方博客的逐条对比 + 据其做的安全修复
├── img/                     # 3 张架构/时序/矩阵 SVG
